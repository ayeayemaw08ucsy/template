...
public class DepItemWriter implements ItemWriter<DepPostingTxn> {

	private String fileDir;

	public DepItemWriter(String fileDir) {
		this.fileDir = fileDir;
	}

	@Override
	public void write(List<? extends DepPostingTxn> items) throws Exception {
		List<DepPostingTxn> txns = new ArrayList<>();
		for (DepPostingTxn txn : items) {
	...
	public void generateDataSourceReport(List<DepPostingTxn> reportList) throws IOException {
		// get jasper template
		ClassPathResource reportResource = new ClassPathResource("templates/dep_report_new.jasper");
		...
		exportReportToPDF(reportResource.getInputStream(), reportParameters, "DEP-REPORT-" + date + ".pdf");
	...
	private ByteArrayResource exportReportToPDF(InputStream targetStream, Map<String, Object> parameters,
		...
		try {
			...
			JasperExportManager.exportReportToPdfStream(jasperPrint,
					new FileOutputStream(fileDir + reportName));
